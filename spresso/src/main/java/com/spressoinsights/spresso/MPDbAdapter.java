...
/**
 * SQLite database adapter for Spresso.
 *
...
class MPDbAdapter {
    ...
    private static final String DATABASE_NAME = "spresso";
    ...
    public MPDbAdapter(Context context, String dbName) {
        if (SpressoConfig.DEBUG) Log.d(LOGTAG, "Spresso Database (" + dbName + ") adapter constructed in context " + context);
    ...
    public int addJSON(JSONObject j, Table table) {
        ...
        if (SpressoConfig.DEBUG) Log.d(LOGTAG, "addJSON " + tableName);
    ...
    public void cleanupEvents(String last_id, Table table) {
        final String tableName = table.getName();
        if (SpressoConfig.DEBUG) Log.d(LOGTAG, "cleanupEvents _id " + last_id + " from table " + tableName);
    ...
    public void cleanupEvents(long time, Table table) {
        final String tableName = table.getName();
        if (SpressoConfig.DEBUG) Log.d(LOGTAG, "cleanupEvents time " + time + " from table " + tableName);
    ...
    /**
     * Returns the data string to send to Spresso and the maximum ID of the row that
